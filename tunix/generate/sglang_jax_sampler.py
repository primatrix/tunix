...
class MappingConfig:
  ...
  to_sglang_jax_mapping: Optional[Dict[str, str]]
  ...
  to_sglang_jax_transpose_keys: Optional[Dict[str, Tuple[int, ...]]]
...
class SglangJaxConfig:
  model_version: str
  ## todo: there maybe no similar parameters in sglang-jax
  # max_model_len: int
...
class SglangJaxSampler(base_sampler.BaseSampler):  # pylint: disable=invalid-name
  ...
  def __init__(
    ...
    # Init sglang-jax model with random weights to speed up bootstrap time, because
    ...
    self.mappings = config.mapping_config.to_sglang_jax_mapping
    self.to_sglang_jax_transpose_keys = config.mapping_config.to_sglang_jax_transpose_keys
    # self.to_hf_hook_fns = config.mapping_config.to_hf_hook_fns

    # TODO(b/434959964) It's not taking effect until sglang Jax backend support
    # lora.
    if (
  ...
  def update_params(
    ...
    utils.transfer_state_with_mappings(
        ...
        transpose_keys=self.to_sglang_jax_transpose_keys,
  ...
  def _sglang_jax_config(self, config: SglangJaxConfig):
    args = {}
    ## todo remove comments when engine enable additional config
  ...
  def _model_runner(self):
    # todo: cannot get model runnner because of subprocess running?
  ...
  def transformer_state(self):
    ...
    if hasattr(self._model_runner, "model"):
      ...
      raise AttributeError("sglang-jax model runner doesn't have model.")
  ...
  ## todo: if there is no necessary to have detokenize
  def detokenize(
      self, input_strings: List[str], request_outputs: Union[Dict, Iterator[Dict]]
  ) -> Tuple[List[str], List[float], List[int]]:
    generations = len(request_outputs)
  ...
  def __call__(
    ...
    outputs = self.engine.generate(
        ...
        sampling_params=[self.sampling_params.convert_to_dict() for _ in prompt_ids],
        # return_logprob = True,
        # logprob_start_len = 0,
        # use_tqdm=True,
    )
